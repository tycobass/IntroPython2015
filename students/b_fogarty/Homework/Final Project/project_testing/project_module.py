...
#    dest = os.dirname(abspath(__file__))
    ...
    def hmda_api(self,state='WA',year=2012,limit=1000):
        """
        Executes get command against CFPB HMDA API, pulling table into memory
        """
        lar = requests.get('https://api.consumerfinance.gov/data/hmda/slice/hmda_lar.json?'+
                           '%24select=action_taken_name%2Capplicant_ethnicity_name'+
                           ...
                           '%27Home+purchase%27+AND+as_of_year+%3D+'+ str(year) +'+AND+applicant_income_000s+%3C+100+AND+state_abbr+%3D+'+
                           ...
                           '%24group=&%24orderBy=&%24limit='+ str(limit) +'&%24offset=0&%24format=json')
    ...
    def data_wrangle(self,frame):
        ...
        redline_report = redline.agg({'minority_flag':'mean',
                           ...
                           'redlining_score':'mean'})
        redline_report.sort('redlining_score',ascending=False)
        self.redline_final = redline_report

    def reports_to_sql(self,frame,report,alias='full_data'):
        """
        Dumps dataframe to sqlite database to store it in a peristent file. 
        DB tables are dropped and remade if they already exist.
        """
        self.state = (frame['state_name'][0])
        cnx = sql.connect(self.state + alias +'.sqlite')
        report.to_sql(self.state+alias,con=cnx,if_exists='replace')
        cnx.commit()
        cnx.close()
