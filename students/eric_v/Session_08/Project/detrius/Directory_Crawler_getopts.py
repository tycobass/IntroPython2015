    ...
    for dirName, subdirs, fileList in os.walk(top_level_folder):
        ...
        for filename in fileList:
            ...
            try:
                ...
                pass
                print ('unprintable counter - ', counter, file=f_log)
...
def printResults(dictionary1, filename_listing, log_file):
    ...
    if len(results) > 0:
        ...
        print ('Potential duplicates found:', file=f_logs)
        print (' Although the names may differ, the contents are identical')
        print ('Although the names may differ, the contents are identical', file=f_logs)
        print ('========================')
        print ('========================', file=f_logs)
        for result in results:
            for subresult in result:
                print('\t\t%s' % subresult)
                print('\t\t%s' % subresult, file=f_logs)
            print('======================')
            print('======================', file=f_logs)
    else:
        print('No duplicate files found.')
        print('No duplicate files found.', file=f_logs)
    f_logs.close()
...
if __name__ == '__main__':
    ...
    if len(folders) >= 1:
        ...
        print ('\n','file_listing can be found in file - ', filename_listing, '\n')
        print (' log file can be found in file - ', log_file , '\n')
    else:
        print('\n', 'Usage: python Directory_Crawler.py -f <filename_listing> -l <log_file> directory1 <directory2>')
